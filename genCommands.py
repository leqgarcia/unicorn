...
class StepCmds:
    ...
    def addAttr(self, attr, attrVal = '', valType = 'int'):
    ...
    def createLayer(self, name, context='misc',laytype='document',polarity='positive'):
        ...
        STR = 'create_layer,layer=%s,context=%s,type=%s,polarity=%s' % (name,context,laytype,polarity)
        self.COM(STR)
        self.getInfo()
        self.getLayers()
        return self.STATUS
    ...
    def addPad(self, x, y, symbol, polarity = 'positive', angle = 0, mirror = 'no', nx = 1, ny = 1 , dx = 0, dy = 0, xscale = 1, yscale = 1, attributes = 'no'):
        ...
        STR = "add_pad,attributes=%s,x=%.6f,y=%.6f,symbol=%s,polarity=%s,angle=%s,mirror=%s,nx=%s,ny=%s,dx=%.3f,dy=%.3f,xscale=%.4f,yscale=%.4f" % (attributes, x, y, symbol, polarity, angle, mirror, nx, ny, dx, dy, xscale, yscale)
        self.COM(STR)
    ...
    def addLine(self, xs, ys, xe, ye, symbol, polarity = 'positive', attributes = 'no'):
        ...
        STR = "add_line,attributes=%s,xs=%.6f,ys=%.6f,xe=%.6f,ye=%.6f,symbol=%s,polarity=%s" % (attributes, xs, ys, xe, ye, symbol, polarity)
        self.COM(STR)
    ...
    def addArc(self, xs, ys, xe, ye, xc, yc, symbol, direction, polarity = 'positive', attributes = 'no'):
        ...
        STR = "add_arc,attributes=%s,xs=%.6f,ys=%.6f,xe=%.6f,ye=%.6f,xc=%.6f,yc=%.6f,symbol=%s,polarity=%s,direction=%s" % (attributes, xs, ys, xe, ye, xc, yc, symbol, polarity, direction)
        self.COM(STR)
    ...
    def addText(self, x, y, text, xSize, ySize, width, mirror = 'no', angle = 0, polarity = 'positive', attributes = 'no'):
        ...
        STR = "add_text,attributes=%s,type=string,x=%.6f,y=%.6f,text=%s,x_size=%.3f,y_size=%.3f,w_factor=%.2f,polarity=%s,angle=%s,mirror=%s,fontname=standard" % (attributes, x, y, text, xSize, ySize, width, polarity, angle, mirror)
        self.COM(STR)
...
class MatrixCmds:
    ...
    def copyRow(self, name, newName, context='misc', newRow=3 ,overwrite = 0,):
        ...
        if (int(self.getRow(newName))) and (overwrite):
            self.job.COM('matrix_delete_row,job='+self.job.name+',row='+str(self.getRow(newName))+',matrix=matrix')
            dest_row = dest_row - 1
            row=self.getRow(name)
            self.job.COM('matrix_copy_row,job='+self.job.name+',row='+str(row)+',matrix=matrix,ins_row='+str(dest_row))
            self.job.COM('matrix_rename_layer,job='+self.job.name+',matrix=matrix,layer='+name+'+1,new_name='+newName)
            self.job.COM('matrix_layer_context,job='+self.job.name+',matrix=matrix,layer='+newName+',context='+context)
            if context == 'board':
                self.getInfo()
                row = self.getRow(newName)
            self.job.COM('matrix_move_row,job='+self.job.name+',row='+str(row)+',matrix=matrix,ins_row='+str(newRow))
            self.getInfo()
            self.job.getInfo()
            self.job.getSteps()
    ...
    def modifyRow(self,currentName,row=0,name='',context='',type='',polarity=''):
        ...
        if row > 0:
            self.job.COM('matrix_move_row,job=' + self.job.name + ',matrix=matrix,row=' + str(curRow) + ',ins_row=' + str(row))
            status = status + int(self.job.STATUS)
        # Change context if desired
        if not (context == ''):
            self.job.COM('matrix_layer_context,job=' + self.job.name + ',matrix=matrix,layer=' + currentName + ',context=' + context)
            status = status + int(self.job.STATUS)
        if not (type == ''):
            self.job.COM('matrix_layer_type,job=' + self.job.name + ',matrix=matrix,layer=' + currentName + ',type=' + type)
            status = status + int(self.job.STATUS)
        if not (polarity == ''):
            self.job.COM('matrix_layer_polar,job=' + self.job.name + ',matrix=matrix,layer=' + currentName + ',polarity=' + polarity)
            status = status + int(self.job.STATUS)
        # Lastly change the name!
        if not (name == ''):
            self.job.COM('matrix_rename_layer,job=' + self.job.name + ',matrix=matrix' + ',layer=' + currentName + ',new_name=' + name)
            status = status + int(self.job.STATUS)
        for step in self.job.steps.keys():
            self.job.steps[step].getInfo()
...
class LayerCmds:
    ...
    def setGenesisAttr(self, name, value):
        ...
        STR = 'set_attribute,type=layer,job=%s,name1=%s,name2=%s,name3=,attribute=%s,value=%s,units=inch' % (self.job.name,self.step.name,self.name,name,str(value))
        self.job.COM(STR)
